...
const navItems = [
  { name: 'Dashboard', href: '/secure-admin', icon: 'home' },
  { name: 'Public Disclosure', href: '/secure-admin/public-disclosure', icon: 'info' },
  { name: 'Site Settings', href: '/secure-admin/site-settings', icon: 'settings' },
  { name: 'Hero Section', href: '/secure-admin/hero', icon: 'image' },
  ...
  { name: 'Gallery', href: '/secure-admin/gallery', icon: 'photo' },
  { name: 'Facilities', href: '/secure-admin/facilities', icon: 'building' },
  { name: 'Admissions', href: '/secure-admin/admissions', icon: 'users' },
  { name: 'Achievements', href: '/secure-admin/achievements', icon: 'trophy' },
  { name: 'Testimonials', href: '/secure-admin/testimonials', icon: 'chat' },
  { name: 'Contact Us', href: '/secure-admin/contact', icon: 'phone' },
  { name: 'Messages', href: '/secure-admin/messages', icon: 'mail' },
  { name: 'SEO Manager', href: '/secure-admin/seo', icon: 'search' },
];
...
const getIcon = (name: string) => {
  const icons: Record<string, JSX.Element> = {
    ...
    start: <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M11.049 2.927c.3-.921 1.603-.921 1.902 0l1.519 4.674a1 1 0 00.95.69h4.915c.969 0 1.371 1.24.588 1.81l-3.976 2.888a1 1 0 00-.363 1.118l1.518 4.674c.3.922-.755 1.688-1.538 1.118l-3.976-2.888a1 1 0 00-1.176 0l-3.976 2.888c-.783.57-1.838-.197-1.538-1.118l1.518-4.674a1 1 0 00-.363-1.118l-3.976-2.888c-.784-.57-.38-1.81.588-1.81h4.914a1 1 0 00.951-.69l1.519-4.674z" />,
...
export default function AdminLayoutClient({ children }: AdminLayoutProps) {
  ...
  return (
    <div className="h-screen bg-slate-900 flex overflow-x-auto overflow-y-hidden">
      {/* Sidebar */}
      <aside className="w-64 flex-shrink-0 bg-slate-800/50 backdrop-blur-xl border-r border-white/10 flex flex-col">
        <div className="p-6 border-b border-white/10">
          <h1 className="font-bold text-white text-lg">Content Manager</h1>
          <p className="text-xs text-gray-500">School Website Admin</p>
        </div>
      ...
      {/* Main Content */}
      <main className="flex-1 flex flex-col min-w-[1024px]">
        <header className="h-16 bg-slate-800/30 border-b border-white/10 flex items-center px-6">
          <h2 className="text-lg font-semibold text-white">
            {navItems.find((item) => pathname === item.href || (item.href !== '/secure-admin' && pathname.startsWith(item.href)))?.name || 'Dashboard'}
          </h2>
        </header>
